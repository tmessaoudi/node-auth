...
router.post("/email/verify", validate(verifyEmailSchema), async (req, res) => {
  ...
  if (!result.length) return res.status(400).json({ message: "Email is incorrect" })

  const user = User.parse(result[0])
  if (user.verified_at) return res.status(400).json({ message: "You are already verified" })
...
router.post("/email/resend", validate(resendEmailSchema), async (req, res) => {
  ...
  const user = User.parse(result[0])
  if (user.verified_at) return res.status(400).json({ message: "Email is incorrect or already verified" })
